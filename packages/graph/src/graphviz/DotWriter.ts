...
export class DotWriter {
    ...
    writeSubgraph(sg: Cluster): string {
        ...
        for (const child of g.subgraphSubgraphs(sgId)) {
            const tpl = this.writeSubgraph(child);
            if (tpl) children.push(tpl);
        }
        for (const child of g.subgraphVertices(sgId)) {
            children.push(this.writeVertex(child));
        }
        for (const child of g.subgraphEdges(sgId)) {
            children.push(this.writeEdge(child));
        }
